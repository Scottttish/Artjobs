...
function InteriorPage() {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      ...
      // Загрузка вакансий из таблицы interior
      const { data: jobData, error: jobError } = await supabase
        .from('interior')
        .select('id, user_id, title, description, published_at, start_date, end_date, price, status, category')
        .eq('category', 'Интерьер');

      if (jobError) {
        console.error('Error fetching jobs from interior:', jobError);
        setError('Ошибка загрузки вакансий: ' + jobError.message);
        setLoading(false);
        return;
      }

      console.log('Fetched jobs from interior:', jobData);
      ...
      if (jobData.length === 0) {
        ...
        if (categoryError) {
          ...
          console.log('Available categories in interior:', [...new Set(allCategories.map(job => job.category))]);
        }
      }
  ...
  return (
    <div className="app-container">
      <main>
        <div className="job-listings-container">
          {jobs.length === 0 ? (
            <p className="no-jobs">Вакансий в категории Интерьер пока нет.</p>
          ) : (
            jobs.map((job) => (
